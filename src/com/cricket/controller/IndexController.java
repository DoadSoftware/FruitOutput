...
import com.cricket.broadcaster.ICC_U19_WORLDCUP_2024;
...
public class IndexController 
{
	...
	public static ICC_U19_WORLDCUP_2024 this_ICC_U19_WorldCup_2024;
	...
	public String outputPage(ModelMap model,
			...
			@RequestParam(value = "qudich", required = false, defaultValue = "") String QT,
			@RequestParam(value = "wagon", required = false, defaultValue = "") String wagon,
					...
					InvocationTargetException, UnknownHostException, ParseException, JAXBException, IOException, URISyntaxException, InterruptedException 
	{
		if(current_date == null || current_date.isEmpty()) {
			...
			session_configuration = new Configuration(selectedMatch, select_broadcaster, 
					speed_select,QT,wagon, select_audio,vizIPAddress, vizPortNumber,"");
			...
			case CricketUtil.ICC_U19_WorldCup_2024 :
				this_ICC_U19_WorldCup_2024 = new ICC_U19_WORLDCUP_2024();
				session_selected_scenes=new ArrayList<Scene>();
				session_selected_scenes.add(new Scene(CricketUtil.ICC_U19_WorldCup_2024_scene,"FRONT_LAYER"));
				session_selected_scenes.get(0).scene_load(CricketFunctions
						.processPrintWriter(session_configuration).get(0), select_broadcaster);
				
				this_ICC_U19_WorldCup_2024.initialize_fruit(CricketFunctions.processPrintWriter(
						session_configuration).get(0), session_match,session_configuration);
				break;
			}
					...
					throws Exception 
	{
		...
		case "READ-MATCH-AND-POPULATE":
			CricketFunctions.getInteractive(session_match);
			...
			case CricketUtil.ICC_U19_WorldCup_2024: 
				...
				if(session_match.getMatch() != null && last_match_time_stamp != new File(CricketUtil.CRICKET_DIRECTORY 
					...
					if(!session_configuration.getPrimaryIpAddress().isEmpty()) {
						this_ICC_U19_WorldCup_2024.updateFruit(session_selected_scenes.get(0), 
						session_match,CricketFunctions.processPrintWriter(session_configuration).get(0),session_configuration);
					}
					last_match_time_stamp = new File(CricketUtil.CRICKET_DIRECTORY + CricketUtil.MATCHES_DIRECTORY 
						+ session_match.getMatch().getMatchFileName()).lastModified();
				...
				if(!session_configuration.getPrimaryIpAddress().isEmpty()) {
					...
					this_speed = CricketFunctions.getCurrentSpeed(CricketUtil.CRICKET_DIRECTORY 
							+ CricketUtil.SPEED_DIRECTORY + CricketUtil.SPEED_TXT, lastSpeed);
					if(this_speed != null) {
						this_ICC_U19_WorldCup_2024.populateSpeed(CricketFunctions.processPrintWriter(session_configuration).get(0),lastSpeed);
						lastSpeed = this_speed;
					}
					this_review=CricketFunctions.getCurrentReview(CricketUtil.REVIEWS, lastReview);
					if(this_review != null) {
						this_ICC_U19_WorldCup_2024.populateReview(CricketFunctions.processPrintWriter(session_configuration).get(0), session_match,lastReview);
						lastReview=this_review;
					}
				}
				break;
		...
		default:
			...
			case CricketUtil.DOAD_FRUIT:
				...
				break;
			case CricketUtil.ICC_U19_WorldCup_2024 :
				this_ICC_U19_WorldCup_2024.ProcessGraphicOption(whatToProcess, session_match, cricketService, 
					CricketFunctions.processPrintWriter(session_configuration).get(0), session_selected_scenes, valueToProcess,session_configuration);
				break;
			}
